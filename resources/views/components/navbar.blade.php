        ...
        <div class="hidden md:block">
          <div class="ml-4 flex items-center md:ml-6">
            ...
            <div class="relative ml-3">
                <div>
                    {{-- buat kondisi semisal user belum login maka keluar tombol login aja --}}
                    @if(Auth::check())
                    <button type="button" @click="isOpen = !isOpen"
                         class="relative flex max-w-xs items-center rounded-full bg-gray-800 text-sm focus:outline-hidden 
                         focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2
                          focus-visible:ring-offset-gray-800 cursor-pointer"
                         ...
                         <img class="size-8 rounded-full"
                              ...
                              alt="" />
                              {{-- buat nama kalau semisal udah login --}}
                              <div class="text-gray-300 text-sm font-medium ml-4">{{ Auth::user()->name }}</div>
                              {{-- arah panah bawah --}}
                 ...
                 <div x-show="isOpen" x-transition:enter="transition ease-out duration-100 transform"
                    ...
                    <form method="POST" action="/logout">
                        @csrf {{-- biar aman dari serangan hacker csrf --}}
                        <button type="submit" class="block px-4 py-2 text-sm
                         text-gray-700 cursor-pointer" role="menuitem"
     ...
     <!-- Mobile menu, show/hide based on menu state. -->
        <div x-show = "isOpen" class="md:hidden" id="mobile-menu">
            ...
            <div class="border-t border-gray-700 pt-4 pb-3">
                {{-- semisal user login atau udah login --}}
                @if(Auth::check())
                ...
                <div class="mt-3 space-y-1 px-2">
                    ...
                    <a href="/profile"
                        class="block rounded-md px-3 py-2 text-base font-medium text-gray-400
                         hover:bg-gray-700 hover:text-white">Profile</a>
                    {{-- dashboard mobile --}}
                    <a href="/dashboard"
                        class="block rounded-md px-3 py-2 text-base font-medium text-gray-400
                         hover:bg-gray-700 hover:text-white">Dashboard</a>
                    {{-- log out mobile --}}
                    <form method="POST" action="/logout">
                        @csrf {{-- biar aman dari serangan hacker csrf --}}
                        <button type="submit" class="block w-full text-start rounded-md px-3 py-2 text-base font-medium text-gray-400
                         hover:bg-gray-700 hover:text-white cursor-pointer" role="menuitem"
                          tabindex="-1" id="user-menu-item-2">Log out</button>
                    </form>
                </div>
                @else
